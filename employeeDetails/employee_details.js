...
function findBySpecializationJavascript() {
    ...
    const javascriptEmployeesDisplay = javascriptEmployees.map(employee => `<p>${employee.id}: ${employee.name} - ${employee.department} - ${employee.salary} - ${employee.specialization}</p>`).join('');
